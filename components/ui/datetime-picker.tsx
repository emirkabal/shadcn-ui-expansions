...
import { add, addYears, format, max } from "date-fns";
...
function genMonthsFromToDate(
  ...
  if (selectedYear === endMonth.getFullYear()) {
    return Array.from({ length: endMonth.getMonth() + 1 }, (_, i) => ({
      value: i,
      label: format(new Date(selectedYear, i), "MMMM", { locale }),
    }));
  } else {
...
function genYearsFromToDate(startMonth: Date, endMonth: Date) {
...
function Calendar({
  ...
  endMonth,
  ...props
}: CalendarProps & { yearRange?: number; selected?: Date }) {
  ...
  const MONTHS = React.useMemo(() => {
    ...
    return startMonth && endMonth && props.selected
      ? genMonthsFromToDate(startMonth, endMonth, currentYear, locale)
      : genMonths(locale);
  }, [currentYear]);
  ...
  const YEARS = React.useMemo(
    () =>
      startMonth && endMonth
        ? genYearsFromToDate(startMonth, endMonth)
        : genYears(yearRange),
    []
  );
            ...
            <div className="inline-flex gap-2 select-none">
              ...
              <Select
                ...
                onValueChange={(value) => {
                  ...
                  if (
                    endMonth &&
                    newDate.getFullYear() === endMonth.getFullYear() &&
                    newDate.getMonth() > endMonth.getMonth()
                  ) {
                    newDate.setMonth(endMonth.getMonth());
                  }
                  props.onMonthChange?.(newDate);
...
>(
  (
    ...
    const minDate = addYears(new Date(), -maxYearsAgo);
    const maxAllowedDate = addYears(new Date(), -minYearsAgo);
    /**
